/**
  ...
  * PORTING NOTE
  * ------------
  * Everything from OLED_Init() onwards is the JiangXie Technology
  * (jiangxiekeji.com) open-source 0.96" SSD1306 driver, V2.0, 2024.10.20,
  * used under its original terms. Only the bottom hardware layer was replaced
  * so that the driver talks to this project's MyI2C bus:
  *
  *   - OLED_W_SCL / OLED_W_SDA / OLED_GPIO_Init and the driver's private I2C
  *     primitives were removed; OLED_WriteCommand / OLED_WriteData now call
  *     MyI2C_*.
  *   - The SCL/SDA pins and the slave address live in Core/main.h.
  *   - The original "wait for the panel supply to settle" software loop became
  *     Delay_ms().
  *   - OLED_IsConnected() was added to report an unplugged panel.
  *
  * Rendering model: the whole frame is composed in OLED_DisplayBuf in RAM and
  * pushed to the panel by OLED_Update() / OLED_UpdateArea().
  ******************************************************************************
  */
/***************************************************************************************
  * 本程序由江协科技创建并免费开源共享
...
#include "stm32f10x.h"
#include "OLED.h"
#include "MyI2C.h"   /* bus driver now lives in Hardware/MyI2C */
#include "Fmt.h"     /* bounded formatting - see the note on OLED_Printf */
#include "main.h"
#include "Delay.h"
#include <string.h>
#include <math.h>
...
/*===========================================================================*/
/*  Hardware layer - SSD1306 on the project's shared software I2C bus         */
/*===========================================================================*/

/** SSD1306 control byte: the following stream is a command. */
#define OLED_CTRL_CMD           0x00U
/** SSD1306 control byte: the following stream is display data. */
#define OLED_CTRL_DATA          0x40U
/** Payload bytes per transfer; keeps the staging buffer small on a 20 KB part. */
#define OLED_DATA_CHUNK         32U

/**
  * @brief  Write one command byte to the panel.
  * @note   A command is only 2 bytes after the address, so it goes out as one
  *         transaction. The result is deliberately ignored: a missing panel is
  *         reported once by OLED_IsConnected() instead of on every command, so
  *         the render hot path stays free of error branching.
  */
static void OLED_WriteCommand(uint8_t Command)
{
    uint8_t payload[2];

    payload[0] = OLED_CTRL_CMD;
    payload[1] = Command;

    (void)MyI2C_Write(OLED_I2C_ADDRESS, payload, 2U);
}

/**
  * @brief  Write Count data bytes, prefixed with the "data" control byte.
  * @note   The control byte is sent inside the same transaction as the payload
  *         to avoid a restart, but the transfer is split into bounded chunks:
  *         OLED_Update() pushes a whole 128-byte page at once, and a single
  *         stack buffer for that plus control byte would be needlessly large.
  *         A NACK also aborts early instead of clocking out bytes nobody reads.
  */
static void OLED_WriteData(uint8_t *Data, uint8_t Count)
{
    uint8_t chunk[OLED_DATA_CHUNK + 1U];
    ...
    while (Count != 0U)
    {
        uint8_t n = (Count > OLED_DATA_CHUNK) ? OLED_DATA_CHUNK : Count;
        uint8_t i;
        ...
        if (MyI2C_Write(OLED_I2C_ADDRESS, chunk, (uint16_t)(n + 1U)) != MYI2C_OK)
        {
            return;     /* panel stopped listening; drop the rest of the frame */
        }
...
/**
  * @brief  Probe the panel on the I2C bus.
  * @return 1 when the SSD1306 acknowledged its address, 0 otherwise.
  */
...
void OLED_Init(void)
{
	MyI2C_Init();				//底层总线初始化（引脚 + 开漏配置）
	Delay_ms(100);				//等待OLED供电稳定，缺此延时偶发初始化失败
...
/**
  * 函    数：整数 atan2，返回角度（度）
  * 参    数：Y X 点的坐标
  * 返 回 值：-180 ~ 180 度，水平向右为 0 度，下方为正，上方为负，顺时针为正
  * 说    明：原实现用 atan2()/3.14*180，会链入软件双精度浮点库（约 2 KB）。
  *           这里用整数 CORDIC 式查表替代：先按 |Y|/|X| 的一阶近似得到
  *           45 度以内的角度，再用比较表校正到 1 度精度，纯整数运算。
  */
static int16_t oled_atan2_deg(int32_t y, int32_t x)
{
	int32_t ax, ay;
	int32_t angle;
	int16_t result;
	...
	/* 45 度以内：angle ≈ 45 * ay / (ax + ay)，最大误差约 1 度 */
	angle = (45 * ay) / (ax + ay);

	/* 校正表：把上面近似的 0/15/30/45 度档位推向真实值 */
	if (ay > ax)
	{
		angle = 90 - angle;			/* 换到以 Y 轴为基准 */
	}
...
{
	int16_t PointAngle;
	PointAngle = oled_atan2_deg((int32_t)Y, (int32_t)X);	/*计算指定点的角度*/
...
/**
  ...
  */
/**
  * 函    数：OLED显示定点数（不依赖浮点库）
  * 参    数：X Y 起始坐标
  * 参    数：Scaled 已放大 10^FraLength 倍的有符号整数
  *           例如显示 25.75（2 位小数）应传入 2575
  * 参    数：IntLength 整数部分位数（不足补空格）
  * 参    数：FraLength 小数部分位数（不足补 0）
  * 参    数：FontSize  字体大小
  * 返 回 值：无
  * 说    明：原作者版本接收 double，会链入软件双精度浮点库（实测约 2 KB，
  *           占 64 KB Flash 的 3 %+）。本工程没有任何一处需要真浮点，
  *           因此改为调用方预先放大为定点整数，显示结果一致而库体积为零。
  *           调用方负责保证 Scaled 不溢出 int32_t，且 FraLength 不超过 9。
  */
void OLED_ShowFixedNum(int16_t X, int16_t Y, int32_t Scaled, uint8_t IntLength, uint8_t FraLength, uint8_t FontSize)
{
	...
	/* 拆分：整数除法与取模，全程整数，无舍入误差 */
	powNum  = OLED_Pow(10, FraLength);
	if (Scaled < 0)
	{
		intPart = (uint32_t)(-(int64_t)Scaled) / powNum;	/*避免 INT32_MIN 取负溢出*/
		fraPart = (uint32_t)(-(int64_t)Scaled) % powNum;
	}
	else
	{
		intPart = (uint32_t)Scaled / powNum;
		fraPart = (uint32_t)Scaled % powNum;
	}

	/* 拼接成完整字符串，避免逐段计算 X 偏移时出错 */
	(void)Fmt_Format(buf, sizeof(buf), "%c%lu.%0*lu",
	                 (Scaled < 0) ? '-' : '+',
	                 (unsigned long)intPart,
	                 (int)FraLength,
	                 (unsigned long)fraPart);
...
/**
  ...
  * @note   Uses the project's own bounded formatter rather than vsprintf.
  *         vsprintf pulled in the whole newlib printf engine plus malloc and
  *         its reentrancy scaffolding - several KB of flash on a 64 KB part -
  *         for what is only ever a short label. Fmt.c covers the integer and
  *         string conversions this display actually uses, and cannot overrun
...
void OLED_Printf(int16_t X, int16_t Y, uint8_t FontSize, const char *format, ...)
{
	...
	va_list arg;
	va_start(arg, format);
	(void)Fmt_VFormat(String, sizeof(String), format, arg);
	va_end(arg);
